...
echo -c %encoder_cfg% -i "D:/seq/%1_%2x%3_%4.yuv" -b %dat_dir%\str_%1_%2_QP%QP%.bin -o %dat_dir%\rec_%1_%2_QP%QP%.yuv -wdt %2 -hgt %3 -f %frames% -fr %4 -q %QP% --IntraPeriod=%6 %add_params% >> %taskfile%
